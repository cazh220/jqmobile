...
listTable.toggle = function(obj, act, id)
{
  ...
  if (res.error == 0)
  {
    obj.src = (res.content > 0) ? '../public/images/goods/yes.gif' : '../public/images/goods/no.gif';
  }
}
...
*查询
*/
listTable.searchgoods = function()
{   
	var args = "?do="+this.query+"" + this.compileFilter();
	window.location.href = this.url+args;
}
...
*查询评论
*/
listTable.searchcomment = function()
{   
	var args = "?do="+this.query+"" + this.compileFilter();
	window.location.href = this.url+args;
}
 ...
 * 导出数据
 */
listTable.exportData = function()
{
	var args = "act=export" + this.compileFilter();
  	Ajax.call(this.url, args, null, "POST", "JSON");
}

/**
 * 删除列表中的一个记录
 */
listTable.remove = function(param, cfm, opt)
{
  if (opt == null)
  {
    opt = "remove";
  ...
  if (confirm(cfm))
  {
    var args = "act=" + opt + "&param=" + param + this.compileFilter();
