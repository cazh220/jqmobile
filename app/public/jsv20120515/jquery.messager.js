 ...
 * $Vesion: 1.0
 *
 * $Id: jquery.messager.js 2554 2011-03-24 02:08:31Z bao $
 */
	this.version = '@1.0';
	this.layer = {'width' : 200, 'height': 110};
	this.title = '信息提示';
	this.label = '点击查看';
	this.time = 5000;
	this.anims = {'type' : 'slide', 'speed' : 600};
	
	this.inits = function(title, text, label){
		if($("#message").is("div")){ return; }
		$(document.body).prepend('<div id="message" style="border:#b9c9ef 1px solid;z-index:100;width:'+this.layer.width+'px;height:'+this.layer.height+'px;position:absolute; display:none;background:#cfdef4;bottom:1pt; right:8pt; overflow:hidden;"><div style="border:1px solid #fff;border-bottom:none;width:100%;height:25px;font-size:12px;overflow:hidden;color:#1f336b;"><span id="message_close" style="float:right;padding:5px 0 5px 0;width:16px;line-height:auto;color:red;font-size:12px;font-weight:bold;text-align:center;cursor:pointer;overflow:hidden;">×</span><div style="padding:5px 0 5px 5px;width:100px;line-height:18px;text-align:left;overflow:hidden;color:#586F8E; ">'+title+'</div><div style="clear:both;"></div></div> <div style="padding-bottom:5px;border:1px solid #fff;border-top:none;width:100%;height:auto;font-size:12px;"><div id="message_content" style="margin:0 5px 0 5px;border:#b9c9ef 1px solid;padding:10px 0 10px 5px;font-size:12px;width:'+(this.layer.width-17)+'px;height:'+(this.layer.height-70)+'px;color:#748096;background:#E1EAF9;text-align:left;overflow:hidden;">'+text+'</div><div id="message_bottom" style="text-align:center;color:red;">'+label+'</div></div></div>');
	};
	this.show = function(title, text, label, time){
		if($("#message").is("div")){ return; }
		if(title==0 || !title)title = this.title;
		if(label==0 || !label)label = this.label;
		this.inits(title, text, label);
