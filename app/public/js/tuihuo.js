...
$(function(){
	...
	$('td.fill').click(function(){
		...
		if($(this).siblings('.goods_name').text() != ""){
			$(this).removeClass('focus');
		}
	});
	
	//条码列下面的input输入框添加class ， 为ajax做准备
	$('td.fill').click(function(){
		if($(this).hasClass('barcode'))
		{
			$(this).addClass('focus').find('input').addClass('barcode');
		}
...
var ajax = function($obj){
	...
	//如果不是goods_sn ， return
	td = $('td.focus');
	//if(!td.hasClass('goods_sn') && !td.hasClass('barcode')) return ;
	...
	if(barcode)
	{
	$('#list td.barcode').each(function(){
		if( $(this).text() == barcode ) {	
			i += 1;
			
			if(i>1) {
				err_sn = '输入的条码：'+barcode+'已存在！';
				$('td.focus').text('');
                $(this).removeClass('focus');
				return false;
			}
	...
	$.ajax({
		   ...
		   data: "goodssn=" + sn + "&type=out&barcode="+barcode+"&agency=1",
		   success: function(msg){
			 	if(!msg.status)
				{
					alert(msg.info);
				...
				$.each(msg.info.stock , function(i , v){
					barcode += '<td class="size_info"><span>'+i+'</span><br /><input type="text" name="'+i+'" value="'+v['barcode']+'" readonly="readonly"></td>';
				});
				barcode += '</tr></table>';
				//$('.barcode').unbind('click');
				...
				td.siblings('.goods_sn').text(msg.info.goods_sn);
				td.siblings('.barcode').html(barcode);
...
var AgencyChange = function(obj,act){
	...
	var barcode   = tr.find('td.barcode').text();
	...
	$.ajax({
		   ...
		   dataType: "json",
		   url: "goods.php?do=GetGoodsInfo",
		   data: "&goodssn="+goods_sn+"&barcode="+barcode+"&agency="+agencyid,
		   success: function(msg){
			 	if(!msg.status)
				{
					alert(msg.info);
...
function export_record(){
    window.location.href = 'tuihuo.php?do=searchList&act=export';
