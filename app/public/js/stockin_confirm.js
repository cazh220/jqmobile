...
function quality_confirm()
{   
	var ar_stockid = new Array();
	$('#list tr td input[type=checkbox]:checked').each(function(){
			  ar_stockid.push($(this).val());
		});	
	...
	$.ajax({
		   type:"POST",
		   dataType:"json",
		   url:"stockin.php?do=Quality",
		   data:"stock_in_id="+ar_stockid,
		   success:function(msg){
			      if(msg.status==1)
				  {
					  alert(msg.info);
				  ...
				  else if(msg.status==2)
				  {
					  ...
					  var supperliersid = $("select option:selected").val();
					  var batchid = $('input[name=batchid]').val();
					  var starttime = $('input[name=start_time]').val();
					  var endtime = $('input[name=end_time]').val();
					  var url = 'audut_status=3&batch='+batch+'&batchid='+batchid+'&start_time='+starttime+'&end_time='+endtime+'&supperliers=0&supperliersid='+supperliersid+'&user=supplier';
					  window.location.href = '../stockin.php?do=searchList&'+url;
//					  location.reload();
					  $('#quality').css("display",'none');
					  $('#shenghe').css("display",'');
