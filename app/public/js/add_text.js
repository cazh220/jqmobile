...
$(function(){  
    ...
    //分页
	$('.page select[name=pages]').change(function(){
		...
		var goods_name = $('input[id=goods_name]').val();
		var cat_id = $('input[id=goods_typeid]').val();
		
		window.location.href = "./standard_costs.php?do=SearchGoods&goods_name="+goods_name+"&currentpage="+page+"goods_cat="+cat_id;

	});	


	$('.addcost').css('backgroundColor', '#FFC');
	$(".addcost").dblclick(function(){
		var val = $(this).text();
		$(this).html("<input type='text' value='"+val+"' size='15' class='standard_cost'>");
		$(this).children(".standard_cost").focus();
		return;
	});	

	$(".standard_cost").live("blur",function(){
		var val = $(this).val();
		var tr = $(this).parents("tr").children();
		var goods_id = $(tr['0']).text(); 
		
				
		$.ajax({ 
		   type: "POST",
		   dataType: "json",
		   url: "standard_costs.php?do=UpdateCost",
		   data: "act=edit&standard_cost="+val+"&goods_id="+goods_id,
		   success: function(msg){			   
			   if(msg.status){
			      	//true
					//alert('编辑成功');
