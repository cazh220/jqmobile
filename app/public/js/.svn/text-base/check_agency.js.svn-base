...
$(function(){
	...
	$('input.numeric').keyup(function(event){
		...
		//次品数量
		var num_bad = $(this).val();
		if(num_bad < 0 || num_bad == '')
		{
			alert('请输入盘点次品数量'+num_bad+'--'+keyCode);
			return false;		
		}
		   ...
		   success: function(msg){
			  ...
			  $.each(msg.data , function(i , v){
					tr += '<td><input type="checkbox" /></td>'+
						  ...
						  '<td style="color:red;">'+v.curr_bad_quantity+'</td>'+
						  '<td class="must_fill_bad">'+num_bad+'</td>'+
						  '<td>'+time+'</td>'+
