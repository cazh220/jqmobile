...
var ajax = function($obj){
	...
	var bad = $('input[name=rejects]').val();
 
	$.ajax({
		   type: "POST",
		   dataType: "json",
		   url: "goods.php?do=GetGoodsInfo",
		   data: "goodssn=" + sn + "&sign="+bad+"&type=out",
		   success: function(msg){
			 	if(!msg.status)
				{
					alert(msg.info);
				...
				$.each(msg.info.stock , function(i , v){
					if(bad == 1)
					{
						size_num += '<option value="'+v['bad_quantity']+'" >'+i+'</option>';
					}
					else
					{
			    ...
			    //各尺寸对应的冻结商品数量
				var size_fnum = '<select name="size_fnum" onchange="changefStockNum(this)"><option value="">请选择</option>';
				$.each(msg.info.stock , function(i , v){
					size_fnum += '<option value="'+v['freeze']+'" >'+i+'</option>';
				});
				size_fnum += '</select>';
				//赋值
				...
				td.siblings('.size_fnum').html(size_fnum);
...
var changefStockNum = function(obj) {

	$(obj).parent().parent().find('.freeze_num').text($(obj).val());
	$(obj).parent().parent().find('.allocate_num').html(0);
}
