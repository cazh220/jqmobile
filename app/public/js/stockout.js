...
$(function(){
	...
	$('#operate_stockout input[name=submit]').click(function(){
		...
		var surplus = false;
		if(act == "lose"){
			surplus = $('#operate_stockout input[name=surplus]').attr('checked') ? 1 : 0 ;
			var conf = confirm(surplus ? '盘盈' : '盘亏');
			if(!conf)return;
		}
		...
		var data = "act="+act+"&stockout_sn="+stockout_sn+"&create_time="+create_time
			+"&surplus="+surplus+"&out_person="+out_person+"&description="+description;
		...
		$(this).attr('disabled',true);
		 //alert(ar_goodsinfo.join);return;
...
var ajax = function($obj){
	...
	$.ajax({
		   ...
		   data: "goodssn=" + sn + "&type=out&agency=1",
		   success: function(msg){
			 	if(!msg.status)
				{
					alert(msg.info);
...
var check = function(obj){
	...
	//if(parseInt(num) > parseInt(stock_sum)){
		if(!$('#operate_stockout input[name=check_on]').val()){
...
function shortage_over(){  
	...
	window.location.href = './stockout.php?do='+act+'&start_time='+starttime+'&end_time='+endtime;
}